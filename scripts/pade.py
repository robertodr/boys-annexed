...
namespace pade {{
/**@{{ Pretabulated coefficients for the [M/N] Padé approximants of the Boys function of orders 1 <= O <= {max_order}.
...
namespace pade {{
/**@{{ [{P}/{Q}] Padé approximant coefficients for Boys function of orders 1 <= O <= {max_order}.*/
{lines}
/**@}}*/
}}  // namespace pade
"""
...
_LINES = """
template <>
inline constexpr auto approximant<{P}, {Q}, {order}>(double*ys, double x, size_t offset) -> void
{{
...
/** Numerator of Padé approximant */
template <>
inline constexpr auto Ps<{P}, {order}>() -> std::array<double, {len_Ps}> {{ return std::array<double, {len_Ps}>{{{{ {ps} }}}}; }}
/** Denominator of Padé approximant */
template <>
inline constexpr auto Qs<{Q}, {order}>() -> std::array<double, {len_Qs}> {{ return std::array<double, {len_Qs}>{{{{ {qs} }}}}; }}
"""
    ...
    with Path(folder / "Pade.cpp").open("w") as fh:
        fh.write(
            _CPP.format(
                ...
                lines="\n".join(
                    [
                        f"case {n}:\n    return cpu::boys_function<M, N, {n}>(xs);"

...
template <auto M, auto N, auto order> inline constexpr auto Ps() -> std::array<double, M+1>;
/** Denominator of Padé approximant */
template <auto M, auto N, auto order> inline constexpr auto Qs() -> std::array<double, N+1>;
}}  // namespace pade
/**@}}*/
...
/** Numerator of Padé approximant */
template <>
inline constexpr auto Ps<{P}, {Q}, {order}>() -> std::array<double, {len_Ps}> {{ return std::array<double, {len_Ps}>{{{{ {ps} }}}}; }}
/** Denominator of Padé approximant */
template <>
inline constexpr auto Qs<{P}, {Q}, {order}>() -> std::array<double, {len_Qs}> {{ return std::array<double, {len_Qs}>{{{{ {qs} }}}}; }}
"""
    ...
    with Path(folder / "Pade.cpp").open("w") as fh:
        fh.write(
            _CPP.format(
                ...
                lines="\n".join(
                    [
                        f"case {n}:\n    return cpu::boys_function<M, N, {n}>(xs.data());"
